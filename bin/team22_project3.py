...
class State:
    ...
    global data
    global clockInput #determines when data can be written

class Combinational:

    def operate(self, arg1, arg2):
        print str(self) + 'recieved Arg1: ' + str(arg1) + ' and' + str(arg2)

    def AffectState(self, address):
        if (address is State):
            print "Affecting state: " + str(address)
        elif (address is int):
            print "Affecting state at address: " + str(bin(address))
        pass
...
class PC(State):
    ...
    data = 96
#The pre-ALU buffer has two entries.  Each entry can store an instruction with its operands.
#The buffer is managed as a FIFO queue
class PreALUQueue(State):
    pass

#The post-ALU buffer has one entry that can store the instruction with the destination register ID
# and the result of the ALU operation
class PostALUBuffer(State):
    pass
#The pre-mem buffer has two entries.  Each entry can store an instruction with its address and data (for STUR).
# It is managed as a FIFO queue.
class PreMemQueue(State):
    pass
...
# Calculate the memory addresses  in this unit.
class MemUnit(Combinational):

    def Load(self):
        #if hits
            #finished after first cycle
            #data written to post mem buffer
        #else
            #operation remains in pre-mem buffer
            #retry next cycle
        pass

    def Store(selfs):
        #(store NEVER goes into post-mem buffer)
        ...
        #else
            #try next cycle
            #if required set is full
                #figure out what to kick out
                #(this may cause a stall)
...
class WBU(Combinational):
    #(can execute 2 write backs in one cycle)
